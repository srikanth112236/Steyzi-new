...
// Self-Service Portal Routes (for admins)
router.get('/subscription/self-service', authenticate, async (req, res) => {
  try {
    const userId = req.user._id;

    // Get current subscription - available to all authenticated users
    const { UserSubscription } = require('../models');
    const subscription = await UserSubscription.findOne({
      userId,
      status: { $in: ['active', 'trial'] }
    ...
    const availablePlans = await costCalculatorService.getPricingTiers(userId);

    // Get usage data
    const usageData = await usageDashboardService.getUsageDashboard(userId, '30d');

    // Get cost breakdown
    const costBreakdown = await usageDashboardService.getUsageCostBreakdown(userId, subscription);
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        availablePlans: availablePlans.success ? availablePlans.tiers : [],
        usageData: usageData.success ? usageData.data : null,
        costBreakdown
      }
    });

  } catch (error) {
    logger.error('Self-service portal error:', error);
    return res.status(500).json({
      success: false,
      message: 'Failed to load self-service portal'
    });
  }
});
