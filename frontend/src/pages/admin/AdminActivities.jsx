...
import { Calendar, Filter, RefreshCw, Search, ChevronLeft, ChevronRight, Download, Info, Activity, Shield, HeadphonesIcon, DollarSign, FileText, Building2, Users, Clipboard, UserCheck } from 'lucide-react';
  ...
  const load = async () => {
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.message || 'Failed to load admin activities');
      }
    } catch (e) {
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">View and track admin-specific actions and logs</p>
          </div>
        </div>
        ...
        <div className="p-4 border-b border-gray-200 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm" value={type} onChange={(e) => { setType(e.target.value); setFilters(f => ({ ...f, page: 1 })); }}>
              ...
              <option value="resident_update">Resident Updated</option>
              ...
              <option value="room_create">Room Created</option>
              <option value="room_update">Room Updated</option>
        ...
        <div className="divide-y divide-gray-200">
          {loading ? (
            <div className="p-10 text-center">
              ...
              <p className="text-gray-500 mt-3">Loading admin activities...</p>
            </div>
          ) : items.length === 0 ? (
            <div className="p-16 text-center">
              <UserCheck className="h-10 w-10 mx-auto text-gray-300 mb-2" />
              <p className="text-gray-500">No admin activities found</p>
              <p className="text-sm text-gray-400 mt-1">Admin activities will appear here when you perform actions</p>
            </div>
          ) : (
            items.map(item => {
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-gray-500">
                        <span>Admin: {item.userEmail}</span>
