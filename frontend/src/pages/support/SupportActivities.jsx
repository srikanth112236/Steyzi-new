...
import { Calendar, Filter, RefreshCw, Search, ChevronLeft, ChevronRight, Download, Info, Activity, Shield, HeadphonesIcon, DollarSign, FileText, Building2, Users, Clipboard, Headphones } from 'lucide-react';
...
const categoryStyles = (cat) => ({
  ...
  icon: (
    ...
    cat === 'system' ? { wrap: 'bg-red-100 text-red-600', Icon: Activity } :
      ...
      <div className="w-full max-w-md bg-white h-full shadow-2xl border-l border-gray-200 p-6 overflow-y-auto">
        ...
        <div className="space-y-3 text-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge color="orange">Support</Badge>
          </div>
...
const SupportActivities = () => {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const res = await activityService.getSupportActivities({ 
        ...filters, 
        q: search, 
        type, 
        category, 
      ...
      if (res.success) {
        ...
        toast.error(res.message || 'Failed to load support activities');
      }
    } catch (e) {
      toast.error('Failed to load support activities');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Support Activities</h1>
            <p className="text-sm text-gray-600">View and track support staff actions and ticket-related logs</p>
          </div>
        </div>
        ...
        <div className="p-4 border-b border-gray-200 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select className="px-3 py-2 border border-gray-300 rounded-lg text-sm" value={type} onChange={(e) => { setType(e.target.value); setFilters(f => ({ ...f, page: 1 })); }}>
              ...
              <option value="ticket_create">Ticket Created</option>
              ...
              <option value="ticket_close">Ticket Closed</option>
        ...
        <div className="divide-y divide-gray-200">
          {loading ? (
            <div className="p-10 text-center">
              ...
              <p className="text-gray-500 mt-3">Loading support activities...</p>
            </div>
          ) : items.length === 0 ? (
            <div className="p-16 text-center">
              <Headphones className="h-10 w-10 mx-auto text-gray-300 mb-2" />
              <p className="text-gray-500">No support activities found</p>
              <p className="text-sm text-gray-400 mt-1">Support activities will appear here when you perform actions</p>
            </div>
          ) : (
            items.map(item => {
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs text-gray-500">
                        <span>Support: {item.userEmail}</span>
